...
//! before the run rather than flown by hand during it — the same call
//! `settings.rs` made for the crew knobs, and for the same reason (the
//! HUD's job is the boat, and a phone screen has no room for anything
//! else). The HUD keeps a read-only panel of the same two indicators, so
//! the conditions stay visible — and one tap away — while driving.
//!
//! It follows the settings menu's overlay pattern: sized off the HUD's
//! own `fs` so it matches the rest of the UI on a phone and a desktop
//! alike, freezing the game while open, and swallowing the press that
//! opened it (`just_opened`). It parts company on ONE point, deliberately:
//! a tap on the scrim APPLIES rather than discards, because unlike a
//! settings slider this modal edits a copy, and a stray tap that silently
//! threw away a set-up scenario would be the worse surprise. Cancel is
//! the explicit way to drop the edits.
//!
...
/// The settings menu's overlay palette, so the two modals read as one UI:
/// a scrim that dims the game without hiding it, and a rimmed card.
const SCRIM: Color = Color::new(0.02, 0.06, 0.09, 0.72);
const CARD_BG: Color = Color::new(12.0 / 255.0, 26.0 / 255.0, 36.0 / 255.0, 245.0 / 255.0);
const CARD_RIM: Color = Color::new(120.0 / 255.0, 200.0 / 255.0, 235.0 / 255.0, 200.0 / 255.0);
...
/// window size — same idiom as `SettingsLayout`, and sized in the HUD's
/// own `fs` units for the same reason: a layout scaled by anything else
/// lands buttons that are either unreadable or untappable at one end of
/// the phone/desktop range.
#[derive(Clone, Copy)]
...
impl ScenarioLayout {
    /// Centred on the screen, `fs` being the HUD's font scale.
    pub fn centred(sw: f32, sh: f32, fs: f32) -> ScenarioLayout {
        let pad = fs * 1.2;
        let gap = fs * 0.6;
        let w = (sw * 0.86).min(fs * 26.0);
        let r = fs * 2.6;
        // Title + subtitle, then the dials with their value and key-hint
        // lines under them, then the two button rows.
        let head = fs * 2.6;
        let dials = r * 2.0 + fs * 2.6;
        let preset_h = fs * 2.2;
        let act_h = fs * 2.4;
        let h = pad * 2.0 + head + dials + gap + preset_h + gap + act_h;
        let card = Rect::new((sw - w) * 0.5, (sh - h) * 0.5, w, h);
        let content = card.w - pad * 2.0;
        let cy = card.y + pad + head + r;
        let bw = (content - gap * (PRESETS.len() as f32 - 1.0)) / PRESETS.len() as f32;
        let py = card.y + pad + head + dials + gap;
        let ay = py + preset_h + gap;
        ...
        ScenarioLayout {
            ...
            presets: std::array::from_fn(|i| {
                Rect::new(card.x + pad + (bw + gap) * i as f32, py, bw, preset_h)
            }),
            apply: Rect::new(card.x + pad, ay, aw, act_h),
            cancel: Rect::new(card.x + pad + aw + gap, ay, aw, act_h),
        }
    }
}
...
pub struct Scenario {
    ...
    /// Set by `open`, cleared by the first `update`. The key press (or the
    /// panel tap) that opens the modal is STILL "pressed" when the
    /// overlay's own input runs later in the same frame — V would close it
    /// again instantly, and the tap would land on the scrim and apply.
    /// (The settings menu learned this first; see its `just_opened`.)
    just_opened: bool,
}
...
impl Scenario {
    ...
    /// A click/tap at `p` on one of the buttons — or on the scrim outside
    /// the card, which closes the modal KEEPING the edits (see the module
    /// note: this modal edits a copy, so the settings menu's
    /// dismiss-on-scrim would silently throw a scenario away). Shared by
    ...
    fn button_at(&mut self, layout: &ScenarioLayout, p: Vec2) -> ScenarioAction {
        ...
        if layout.apply.contains(p) || !layout.card.contains(p) {
    ...
    pub fn update(&mut self, layout: &ScenarioLayout) -> ScenarioAction {
        ...
        // Enter applies, and so does V — closing with the key that opened
        // it keeps the edits, like tapping the scrim.
        if is_key_pressed(KeyCode::Enter) || is_key_pressed(KeyCode::V) {
            return ScenarioAction::Apply;
        }
    ...
    pub fn draw(&self, layout: &ScenarioLayout) {
        ...
        // what the conditions are going to blow across.
        draw_rectangle(0.0, 0.0, screen_width(), screen_height(), SCRIM);
        draw_rectangle(card.x, card.y, card.w, card.h, CARD_BG);
        draw_rectangle_lines(card.x, card.y, card.w, card.h, 2.0, CARD_RIM);

        let pad = fs * 1.2;
        draw_text("SCENARIO", card.x + pad, card.y + pad + fs, fs * 1.15, TEXT_COL);
        draw_text(
            "the conditions this run is sailed in",
            card.x + pad,
            card.y + pad + fs * 2.1,
            fs * 0.78,
            DIM_COL,
        ...
        for (dial, vel, frac, col, label, keys, grabbed) in [
            ...
            centred_text(&label, dial.cx, dial.cy + dial.r + fs * 1.25, fs * 0.85, col);
            centred_text(keys, dial.cx, dial.cy + dial.r + fs * 2.2, fs * 0.65, DIM_COL);
        }

        for (rect, preset) in layout.presets.iter().zip(PRESETS.iter()) {
            fitted_button(*rect, preset.label, fs * 0.8);
        }
        fitted_button(layout.apply, "Apply [Enter]", fs * 0.9);
        fitted_button(layout.cancel, "Cancel [Esc]", fs * 0.9);
    }
}
...
/// A button in the HUD's own style (`crate::hud_button`), with the label
/// shrunk until it fits — the keel editor's rule, and this modal needs it
/// for the same reason: "Wind v tide [4]" has to survive a phone-width
/// card without being clipped.
fn fitted_button(rect: Rect, label: &str, size: f32) {
    let mut size = size;
    while measure_text(label, None, size as u16, 1.0).width > rect.w - size * 0.6 && size > 8.0 {
        size -= 1.0;
    }
    crate::hud_button(rect, label, size, false);
}
