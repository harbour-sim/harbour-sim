...
impl CrewLimits {
    ...
    /// crew. `f32::clamp` returns NaN when `self` is NaN rather than a
    /// bound of the range, so a non-finite field is replaced by its
    /// default first — otherwise a NaN `reach` would compare false
    /// against every distance check downstream and pass as unlimited.
    pub fn clamped(self) -> CrewLimits {
        ...
        CrewLimits {
            pass_speed: finite(self.pass_speed, LINE_PASS_SPEED)
                .clamp(LINE_PASS_SPEED_MIN, LINE_PASS_SPEED_MAX),
            haul_kg: finite(self.haul_kg, LINE_HAUL_KG)
                .clamp(LINE_HAUL_KG_MIN, LINE_HAUL_KG_MAX),
            reach: finite(self.reach, LINE_REACH).clamp(LINE_REACH_MIN, LINE_REACH_MAX),
        }
    }
...
mod tests {
    ...
    /// `f32::clamp` returns NaN when `self` is NaN rather than a bound of
    /// the range — a settings field corrupted to NaN would otherwise
    /// survive `clamped()` unchanged, and a NaN `reach` compares false
    /// against every distance check, letting a throw of any length
    /// through instead of being refused as out of reach. `tick` calls
    /// `clamped()` once up front (like the throttle/helm clamps) and
    /// hands the result to `apply_command`, so this checks the same
    /// combination: a corrupt `CrewLimits`, clamped, then used.
    #[test]
    fn a_corrupt_setting_is_replaced_rather_than_left_nan() {
        let corrupt = CrewLimits { pass_speed: f32::NAN, haul_kg: f32::NAN, reach: f32::NAN };
        let clamped = corrupt.clamped();
        assert!(clamped.pass_speed.is_finite());
        assert!(clamped.haul_kg.is_finite());
        assert!(clamped.reach.is_finite());
        ...
        let cmd = LineCommand::MakeFast {
            fairlead: Fairlead::PortBow,
            anchor: Anchor::Shore {
                pos: Vec2::new(LINE_REACH_MAX + 1.0, 0.0),
                kind: ShoreKind::Pole,
            },
        };
        crate::line::apply_command(&mut lines, &mut id, cmd, clamped, &[], |_| Vec2::ZERO, shore_at);
        assert!(lines.is_empty(), "a NaN reach must not be read as unlimited");
    }
